...
function EditStudent() {
    ...
    const currentEductaionEdit = async (data) => {
        ...
        if (token) {
            ...
            const result = await axios.post(`${process.env.REACT_APP_API_URL}/api/edit-eductaion-detail?id=${location.state.userId}`, payload, { headers: { Authorization: token } });
            if (result.data.success) {
                profileGet()
                // alert(data.data.msg)
                toast.success(result.data.msg, {
    ...
    const bankEdit = async (data) => {
        ...
        if (token) {
            ...
            const result = await axios.post(`${process.env.REACT_APP_API_URL}/api/edit-bank?id=${location.state.userId}`, payload, { headers: { Authorization: token } });
            if (result.data.success) {
                profileGet()
                // alert(data.data.msg)
                toast.success(result.data.msg, {
