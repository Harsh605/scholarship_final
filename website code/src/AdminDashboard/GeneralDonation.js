...
function GeneralDonation() {
    ...
    const getScheme = async () => {
        ...
        if (adminToken) {
            ...
            const data = await axios.get(`${process.env.REACT_APP_API_URL}/api/admin-general-donation-scheme-list`, { params: query, headers: { Authorization: adminToken } });
    ...
    const onSubmit = async (data) => {
        ...
        console.log("addScheme.duration")
        if (!addScheme.duration) {
            setMessage("Select duration.")
            return
        }
        ...
        if (adminTtoken) {
            const payload = {
                ...
                duration: addScheme.duration,
            }

            const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/create-general-scheme`, payload, { headers: { Authorization: adminTtoken } });
            if (res.data.success) {
                setOpenDialog(false)
                setAddScheme({})
                getScheme({ keyword: '' })
            ...
            <div className="w-full px-0 md:px-6 py-2">
                <div className="px-4 md:px-10 py-4 md:py-7 bg-gray-100 rounded-tl-lg rounded-tr-lg">
                    <div className="sm:flex items-center justify-between">
                        <p className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-gray-800">General Donation</p>
                        <div>
                ...
                <div className="bg-white shadow px-4 md:px-10 pt-4 md:pt-7 pb-5 overflow-y-auto">
                    <table className="w-full whitespace-nowrap">
                        <thead>
                            <tr className="h-16 w-full text-sm leading-none text-gray-800">
                                ...
                                <th className="font-normal text-left pl-12">No. of Student
                                    {/* <button className='ml-1' style={{ width: "1.5rem", height: "1.2rem", backgroundColor: "white" }}>
                                        <img name='amount' src={caret} alt='' className={className('w-2.5 rotate-180 ', { 'opacity-20': sort.amountUpArrow })} onClick={(e) => handleSort(e, 'asc', 'amountUpArrow')} />
                                        <img name='amount' src={caret} alt='' className={className('w-2.5', { 'opacity-20': sort.amountDownArrow })} onClick={(e) => handleSort(e, 'desc', 'amountDownArrow')} />
                                    </button> */}
                        ...
                        <tbody className="w-full">
                            {scheme && scheme?.length > 0 && scheme?.map((cur, index) => (
                                <tr
                                    key={cur.id}
                    ...
                    {scheme && scheme?.length === 0 && <div className='border p-2' style={{ textAlign: 'center' }}>
                        No Data Found.
                    </div>}
                    ...
                    <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
                        <DialogTitle>Add General Donation</DialogTitle>
                        <DialogContent>
                            <TextField
                                fullWidth
                            ...
                            <NativeSelect fullWidth
                                ...
                                variant="outlined"
                                sx={{ marginTop: "20px" }}
                                onChange={(e) => { setAddScheme({ ...addScheme, 'duration': e.target.value }); if (message === "Select Duration.") setMessage(''); }}>
                                <option value="" disabled selected>Select Duration</option>
                                {['1 month', '6 month', '12 month']?.map((data, index) => (
                            ...
                            <div style={{ color: 'red', textAlign: "center" }}>{errors?.schemeName?.message || message || errors?.amount?.message || errors?.amount?.noOfStudent || ''}</div>
                        </DialogContent>
                        <DialogActions>
