...
const ForgotPassword = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const result = await axios.post(`${process.env.REACT_APP_API_URL}/api/forgot-password`, payload);
    if (result.data.success) {
      reset()

      navigate('/verify-otp', { state: { email: data.email, text: 'forgot-password' } })
      setTimeout(() => {
